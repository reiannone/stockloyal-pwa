...
export default function FillBasket() {
  ...
  const {
    amount: initialAmount = parseFloat(localStorage.getItem("basket_amount") || "0"),
    pointsUsed: initialPoints = parseInt(localStorage.getItem("basket_pointsUsed") || "0", 10),
  } = location.state || {};
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error("[FillBasket] Failed to load member picks:", err);
      }
  ...
  const loadMyActiveList = async () => {
    ...
    try {
      ...
      if (!data?.success) {
        console.error("[FillBasket] Failed to load My Basket:", data?.error);
        setMyActiveListData([]);
      ...
      console.error("[FillBasket] My Basket load error:", err);
      setMyActiveListData([]);
    } finally {
      setLoadingMyActiveList(false);
    }
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.error("[FillBasket] fetch wallet error:", e);
  ...
  useEffect(() => {
    if (wallet?.conversion_rate) {
      ...
      if (r > 0) {
        setConversionRate(r);
        console.log(`[FillBasket] Tier conversion rate: ${r} (tier: ${wallet.member_tier || localStorage.getItem("memberTier") || "default"})`);
        return;
      }
    }
  ...
  const handleSaveToPicks = async (symbol) => {
    ...
    try {
      ...
      console.error("[FillBasket] save pick error:", err);
  ...
  const handleRemoveFromPicks = async (symbol) => {
    ...
    try {
      ...
      console.error("[FillBasket] remove pick error:", err);
  ...
  const handleCategoryClick = async (cat, scrId) => {
    ...
    try {
      ...
      console.error("[FillBasket] proxy error:", err);
  ...
  const handlePopularMemberPicks = async () => {
    ...
    try {
      ...
      console.error("[FillBasket] popular member picks error:", err);
  ...
  const handleMyPicks = async (autoLoadToBasket = false) => {
    ...
    try {
      ...
      console.error("[FillBasket] My Basket error:", err);
  ...
  const loadMoreStocks = async () => {
    ...
    try {
      ...
      console.error("[FillBasket] load more error:", err);
      setHasMore(false);
    } finally {
  ...
  const handleSymbolSearch = async () => {
    ...
    try {
      ...
      console.error("[FillBasket] search error:", err);
  ...
  return (
    <div className="wallet-container" style={{ paddingBottom: "100px" }}>
      ...
      {/* ✅ Compact investment summary bar (links back to PointsSlider to change) */}
      <div
        onClick={() => navigate("/points-slider", { state: { amount: cashValue, pointsUsed: selectedPoints } })}
        style={{
          background: "#eff6ff",
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: "0.7rem", color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.5px" }}>Points</div>
          ...
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: "0.7rem", color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.5px" }}>Cash</div>
      ...
      {cashLimitError && (
        <p className="form-error" style={{ marginBottom: "0.5rem", textAlign: "center" }}>
          {cashLimitError}
        </p>
      )}
